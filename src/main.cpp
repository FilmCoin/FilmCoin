...
int64 nChainStartTime = 1406613600;
...
const string strMessageMagic = "Filmcoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight == 1){
	       nSubsidy = 202500 * COIN; // first 1 blocks for 1.5%
	       return nSubsidy + nFees;
	  }
	  else if(nHeight >= 2 && nHeight <= 130){
	  	   nSubsidy = 10 * COIN; // 88coins for per block
	  	   return nSubsidy + nFees;
	  }
	  else if(nHeight >= 131 && nHeight <= 1440){
	  	   nSubsidy = 625 * COIN; // 308coins for per block
	  	   return nSubsidy + nFees;
	  }
	  else if(nHeight >= 1441 && nHeight <= 5760){
	  	   nSubsidy = 525 * COIN; // 308coins for per block
	  	   return nSubsidy + nFees;
	  }
	  else if(nHeight >= 5761 && nHeight <= 10080){
	  	   nSubsidy = 425 * COIN; // 308coins for per block
	  	   return nSubsidy + nFees;
	  }
	  else if(nHeight >= 10080 && nHeight <= 14400){
	  	   nSubsidy = 325 * COIN; // 308coins for per block
	  	   return nSubsidy + nFees;
	  }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Filmcoin
    bool fStrictPayToScriptHash = true; // Always active in Filmcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Filmcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Filmcoin is good coin wawawa......";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x3047787fcf6eb32e72d8dc0cc829c861962c87535b93162b9eddfc8e76d353bf"));
